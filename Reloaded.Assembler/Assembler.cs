...
/* Unmerged change from project 'Reloaded.Assembler (net5.0)'
Before:
using System;
...
using Reloaded.Assembler.Definitions;
using Reloaded.Memory.Buffers;
using System.Memory.Sources;
using System;
using System.Collections.Generic;
using System.Diagnostics;
using System.IO;
using System.Reflection;
using Reloaded.Runtime.InteropServices;
using Reloaded.Text;
using Reloaded.Threading;
*/

/* Unmerged change from project 'Reloaded.Assembler (net6.0)'
Before:
using System;
using System.Collections.Generic;
using System.Diagnostics;
...
public unsafe class Assembler : IDisposable
{
    ...
    public byte[] AssembleFile(string filePath) => Assemble(File.ReadAllLines(filePath));
    ...
    private string GetFasmDLLPath()
    {
        ...
        string asmDirectoryFasm86;
        string asmDirectoryFasm64;
        //check if packed with Costura with 
        if (assemblyDirectory.Contains("Costura"))
        {
            asmDirectoryFasm86 = Path.Combine(assemblyDirectory, Path.Combine("32", FASM86DLL));
            asmDirectoryFasm64 = Path.Combine(assemblyDirectory, Path.Combine("64", FASM64DLL));
        } else
        {
        ...
        throw new FasmWrapperException($"Appropriate FASM DLL for X86/64 has not been found in either current or library directory. {asmDirectoryFasm86}, {asmDirectoryFasm64}");
    }
    ...
    {
        var allocationProperties = _bufferHelper.Allocate(textSize, 1, int.MaxValue, retryCount);
    ...
    {
        var allocationProperties = _bufferHelper.Allocate(resultSize, 1, int.MaxValue, retryCount);
